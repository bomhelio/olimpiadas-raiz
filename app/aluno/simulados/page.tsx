...
function StatusBadge({ status }: { status: string | null }) {
  if (!status)
    return <span className="text-xs text-muted-foreground">Não iniciado</span>;
  if (status === "pausado")
    return <span className="rounded-full bg-amber-500/10 px-2 py-0.5 text-xs font-semibold text-amber-400">Pausado</span>;
  if (status === "em_andamento")
    return <span className="rounded-full bg-emerald-500/10 px-2 py-0.5 text-xs font-semibold text-emerald-400">Em andamento</span>;
  return <span className="rounded-full bg-sky-500/10 px-2 py-0.5 text-xs font-semibold text-sky-400">Concluído</span>;
}
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      ...
                      <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${presencial ? "bg-violet-500/10 text-violet-400" : "bg-sky-500/10 text-sky-400"}`}>
                        {presencial ? "Presencial" : "Online"}
                      </span>
                      {s.duracao_minutos && (
                        <span className="text-xs text-muted-foreground">⏱ {fmtDuracao(s.duracao_minutos)}</span>
                      )}
                    </div>
                    ...
                    {s.data_hora && (
                      <p className="text-xs text-muted-foreground mt-1">{fmtDateTime(s.data_hora)}</p>
                    )}
                    {s.descricao && (
                      <p className="text-sm text-muted-foreground mt-2 line-clamp-2">{s.descricao}</p>
                    )}
                    {s.sessao && (
                  ...
                  <div className="flex flex-col gap-2 shrink-0">
                    ...
                    {concluido && (
                      <Link href={`/aluno/simulados/${s.id}/relatorio`}
