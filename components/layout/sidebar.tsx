...
// Pages whose search params should be persisted across navigation
const PERSIST_BASES = ["/dashboard", "/olimpiadas"] as const;
...
function SidebarContent() {
  ...
  // Persist current page params to localStorage whenever they change
  useEffect(() => {
    for (const base of PERSIST_BASES) {
      const isHere = base === "/dashboard" ? pathname === base : pathname.startsWith(base);
      if (isHere) {
  ...
  return (
    <nav className="flex flex-col py-4">
      {items.map((item) => {
        ...
        const isActive =
          basePath === "/dashboard" ? pathname === "/dashboard" : pathname.startsWith(basePath);
